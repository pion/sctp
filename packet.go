...
func (p *packet) marshal(doChecksum bool) ([]byte, error) {
	...
	// RFC 9653 section 5.2: send a correct CRC32c when the packet contains INIT or
	// COOKIE ECHO. Otherwise, if the caller indicates ZCA is active for this
	// path (doChecksum==true), emit zero.
	// doChecksum here means zero is allowed for this path.
	if doChecksum {
		for _, c := range p.chunks {
			switch c.(type) {
