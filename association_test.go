...
func TestAdaptiveBurst_BudgetAndRefill(t *testing.T) {
	...
	// First "ACK" refill: round 0 -> use burstInitMTUs (default 4.0 => ceil=4),
	// test a smaller budget to assert gating precisely.
	a0.burstBudgetMTUsRemaining = 2

	var pkts [][]byte
	pkts = a0.gatherOutboundDataAndReconfigPackets(pkts)
	require.Len(t, pkts, 2, "should not exceed per-ACK budget (2 MTUs)")
	require.Equal(t, 0, a0.burstBudgetMTUsRemaining)

	// Emulate next SACK arrival refill (handleSack())
	refill := a0.currentBurstLimitMTUsLocked(time.Now())
	require.GreaterOrEqual(t, refill, 1)
	a0.burstBudgetMTUsRemaining = refill
