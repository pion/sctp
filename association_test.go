...
func TestAssocResetResetsInterleavingCounters(t *testing.T) {
	...
	t.Run("late outbound reset response keeps a reopened stream's counters", func(t *testing.T) {
		const (
			si  uint16 = 1
			rsn uint32 = 2
		)
...
// After both directions of a stream reset complete, the peer may reuse the
// stream identifier at once. If our copy of the answer to our own outgoing
// reset arrives after the new generation's first messages (here: it is lost
// and only the answer to the retransmitted request gets through), it must not
// reset the new stream's sequence numbers. Otherwise the next message repeats
// an SSN/MID the peer has already delivered and is never read.
...
// closed by (I-)FORWARD-TSN rather than DATA; the reset must still complete.
...
func testStreamResetCompletesAfterForwardTSN( //nolint:cyclop,gocognit
	...
	server.OnStreamResetComplete(func(id uint16) {
		serverCompleted <- streamResetEvent{id: id}
	})
	...
	require.NoError(t, serverStream.Close())
	require.Equal(t, streamResetEvent{id: streamID}, waitForResetEvent(t, bridge, serverCompleted))
	require.Equal(t, streamResetEvent{id: streamID}, waitForResetEvent(t, bridge, clientCompleted))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			response, err := assoc.handleReconfigParam(&paramOutgoingResetRequest{
				reconfigRequestSequenceNumber: assoc.peerNextRSN,
				senderLastTSN:                 101,
				streamIdentifiers:             []uint16{streamID},
			})
			require.NoError(t, err)
			...
			require.Equal(t, reconfigResultSuccessPerformed, resetResponseResult(t, replies[0]))
			assert.NotContains(t, assoc.streams, streamID)
			assert.Empty(t, assoc.reconfigRequests)
