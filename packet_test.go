...
func FuzzPacket_StrictRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		var p packet
		if err := p.unmarshal(true, data); err != nil {
			return // skip invalid packets
		}

		out, err := p.marshal(true)
		...
		var q packet
		assert.NoError(t, q.unmarshal(true, out), "strict unmarshal after marshal")
	})
}
...
func TestRFC9653_SenderRule_ZeroAllowedWhenNoRestrictedChunk(t *testing.T) {
	...
	// a strict receiver verifies the checksum. a corrupted/invalid CRC32c must be rejected.
	strict, err := pak.marshal(true)
	assert.NoError(t, err)
	binary.LittleEndian.PutUint32(strict[8:], binary.LittleEndian.Uint32(strict[8:])^1)
...
func FuzzRFC9653_SenderAndReceiverRules(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, includeRestricted bool, seed int64) {
		...
		// Strict receiver (doChecksum=true) verifies the checksum, including 0.
		if sum == 0 {
			if computeCRC(raw) != 0 {
				assert.Error(t, q.unmarshal(true, raw), "strict receiver must reject an incorrect zero checksum")
			} else {
				assert.NoError(t, q.unmarshal(true, raw), "strict receiver must accept a valid zero CRC32c")
			}
		} else {
